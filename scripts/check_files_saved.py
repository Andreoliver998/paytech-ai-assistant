...
print("[check] DB_PATH:", DB_PATH)
print("[check] UPLOADS_DIR:", UPLOADS_DIR)

disk_files = []
if UPLOADS_DIR.exists():
    disk_files = [p for p in UPLOADS_DIR.iterdir() if p.is_file()]

print("\n[check] uploads no disco:", len(disk_files))
for p in sorted(disk_files)[-10:]:
    print(" -", p.name)

if not DB_PATH.exists():
    print("\nERRO: DB nao existe:", DB_PATH)
    raise SystemExit(1)
...
print("\n[check] registros na tabela files:", files_count)
...
print("\n[check] ultimos registros em files:")
if not rows:
    print(" (vazio)")
else:
    for r in rows:
...
print("\n[check] registros na tabela kb_chunks:", chunks_count)
print("\n[check] sessoes no banco:", sessions_count)
print("[check] mensagens no banco:", messages_count)
